...
resource "aws_route_table_association" "web" {
  count      = length(var.web_subnets_cidr)
  route_table_id = lookup(element(aws_route_table.web, count.index ), "id", null) # aws_route_table.web[count.index].id
...
resource "aws_route_table_association" "app" {
  count      = length(var.app_subnets_cidr)
  route_table_id = lookup(element(aws_route_table.app, count.index ), "id", null) # aws_route_table.app[count.index].id
...
resource "aws_route_table_association" "db" {
  count      = length(var.db_subnets_cidr)
  route_table_id = lookup(element(aws_route_table.db, count.index ), "id", null) # aws_route_table.db[count.index].id
